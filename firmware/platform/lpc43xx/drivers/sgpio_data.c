...
bool sgpio_isr_necessary_for_function(sgpio_function_t *function)
{
	switch (function->mode) {
		...
		// XXX: hack; this should have an interrupt to shuttle data out
		case SGPIO_MODE_STREAM_BIDIRECTIONAL:
...
{
	interrupt_service_routine_t generated_isr = NULL;
	...
	if (sgpio_isr_necessary_for_function(&sgpio->functions[0])) {
		...
		generated_isr = sgpio_generate_isr_for_function(&sgpio->functions[0]);
	...
	if(generated_isr) {
		platform_set_interrupt_handler(SGPIO_IRQ, generated_isr);
	} else {
		platform_set_interrupt_handler(SGPIO_IRQ, empty_isr);
	}
